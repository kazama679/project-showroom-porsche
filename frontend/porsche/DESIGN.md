...
- Porsche Red (`#DA291C`) used with extreme sparseness — accent, not atmosphere
...
### Primary
- **Porsche Red** (`#DA291C`): Primary accent and CTA color. Used for key action triggers and brand moments where maximum visual authority is needed.
...
### Secondary & Accent
- **Dark Red** (`#B01E0A`): Deeper variant of Porsche Red for hover/pressed states and high-contrast contexts — adds dimensionality to the brand color without introducing a new hue (--f-color-accent-90)
...
### Semantic & Accent
- **Warning Red** (`#F13A2C`): Accessible warning state, intentionally distinct from brand red so semantic alerts do not become brand decoration
...
- **Link Hover Blue** (`#3860BE`): Interactive hover state for text links — a dignified navy-blue that signals interactivity without competing with Porsche Red
...
- Default: bg `#DA291C` (Porsche Red), text `#FFFFFF`, borderRadius 2px, padding 12px 10px
- Used for: Newsletter subscribe, primary conversion actions on dark backgrounds
- The only button that uses Porsche Red — reserved for maximum visual priority
...
Porsche treats white space as a gallery wall. Each section — whether a vehicle render on black void or a detail story on neutral gray — is given its own room to breathe. The alternating black/white sections create a pacing rhythm: dark = immersive moment, white = editorial content, dark = immersive moment. This cadence makes scrolling feel like turning pages in a luxury publication. White space between editorial cards is moderate because Porsche is telling stories, not exhibiting isolated objects.
...
### Do
- Use Porsche Red (`#DA291C`) sparingly — only for primary CTAs and brand-critical moments. Its power comes from restraint
...
### Don't
- Scatter Porsche Red across the interface as decoration — it's a CTA signal, not a theme color
...
- Override semantic state colors with brand colors — warning, success, and info colors must remain meaningfully distinct from brand red
...
- Primary CTA: "Porsche Red (#DA291C)"
...
6. Reserve Porsche Red for ONE element per screen — if red appears in more than one place, it loses its authority
