...
export default function CarModelOptionsPage() {
  ...
  return (
    <PageLayout title={t('admin.car_model_options_management')} subtitle={t('admin.car_model_options_subtitle')}
      actions={
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}
        title={editingAssignment ? t('admin.edit_car_model_option') : t('admin.add_car_model_option')}
        size="md"
        footer={<>
          <Button variant="secondary" onClick={() => setIsModalOpen(false)} disabled={saving}>{t('common.cancel')}</Button>
          <Button variant="primary" onClick={handleSave} loading={saving}>{editingAssignment ? t('admin.update') : t('admin.create')}</Button>
        </>}>
        <div className="space-y-4">
          
          <div className="space-y-2">
            <label className="block text-sm font-medium text-near-black dark:text-light-gray-surface">
              {t('admin.model_name')} <span className="text-brand-red">*</span>
            </label>
            <select
              ...
              required
            >
              <option value="" disabled>{t('admin.select_model')}</option>
              {carModels.map((m) => (
          ...
          <div className="space-y-2">
            <label className="block text-sm font-medium text-near-black dark:text-light-gray-surface">
              {t('admin.option_item_name')} <span className="text-brand-red">*</span>
            </label>
            <select
              ...
              required
            >
              <option value="" disabled>{t('admin.option_item_name')}</option>
              {optionItems.map((opt) => (
