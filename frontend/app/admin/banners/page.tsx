...
export default function BannersPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingBanner) {
        ...
        showAlertMessage(t('admin.banner_updated'), 'success')
      } else {
        await homeBannerService.create(formData)
        showAlertMessage(t('admin.banner_created'), 'success')
      }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      showAlertMessage(t('admin.banner_deleted'), 'success')
  ...
  return (
    <PageLayout
      ...
      subtitle={t('admin.banners_subtitle')}
      actions={
        <div className="flex items-center gap-3">
          <div className="relative hidden sm:block">
            ...
            <input
              type="text"
              placeholder={t('admin.search_banners')}
              value={searchKeyword}
          ...
          <select
            ...
            className="px-3 py-2 text-sm border border-[#D2D2D2] dark:border-[#404040] rounded-[2px] bg-white dark:bg-[#303030] text-[#181818] dark:text-white outline-none focus:border-[#DA291C] transition-colors"
          >
    ...
    >
        ...
        <div className="bg-white dark:bg-[#303030] border border-[#D2D2D2] dark:border-[#303030] rounded-[2px] p-6">
          <DataTable
            columns={[
              ...
              { key: 'title', label: t('admin.banner_title'), sortable: true },
              {
                key: 'type',
                label: t('admin.banner_type'),
                render: (v: any) =>
                  v === 'HERO' ? (
              ...
              { key: 'carModelName', label: t('admin.banner_car_model'), render: (v: any) => v || '—' },
              { key: 'displayOrder', label: t('admin.banner_display_order'), align: 'center' },
              {
                key: 'isActive',
                ...
                render: (v: any) =>
                  v ? (
                    <span className="text-green-600 font-semibold">{t('admin.active')}</span>
                  ) : (
                    <span className="text-red-500 font-semibold">{t('admin.inactive')}</span>
                  ),
              },
              ...(isAdmin
                ? [
      ...
      <Modal
        ...
        title={editingBanner ? t('admin.edit_banner') : t('admin.add_banner')}
        ...
        footer={
          <>
            ...
            <Button variant="primary" onClick={handleSave} loading={saving}>
              {editingBanner ? t('admin.update') : t('admin.create')}
            </Button>
          </>
        }
      >
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            />
            <Select
              label={t('admin.banner_car_model')}
              placeholder={t('admin.select_car_model')}
          ...
          {formData.type === 'HERO' ? (
            <FormInput
              label={t('admin.banner_video_url')}
            ...
            <FormInput
              label={t('admin.banner_image_url')}
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormInput
              label={t('admin.banner_display_order')}
              type="number"
      ...
      <Modal
        ...
        title={t('admin.banner_confirm_delete')}
        size="sm"
        footer={
          <>
      ...
      >
        <div className="flex items-start gap-4">
          ...
          <div>
            <p className="text-sm text-[#181818] dark:text-[#D2D2D2]">{t('admin.banner_confirm_delete_msg')}</p>
            {deletingBanner && (
              <p className="text-sm font-semibold text-[#181818] dark:text-white mt-2">{deletingBanner.title}</p>
            )}
          </div>
        </div>
      </Modal>
