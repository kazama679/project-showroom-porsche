...
export default function OptionsPage() {
  ...
  return (
    <PageLayout title={t('admin.options_management')} subtitle={t('admin.options_subtitle')}
      actions={
        ...
        <div className="bg-white dark:bg-[#303030] border border-[#D2D2D2] dark:border-[#303030] rounded-[2px] p-6">
          <DataTable
            columns={[
              ...
              { key: 'name', label: t('admin.option_name'), sortable: true },
              { key: 'displayOrder', label: t('admin.option_display_order'), align: 'center', render: (v: any) => v ?? '—' },
              ...(isAdmin ? [{
                key: 'actions' as keyof OptionCategory, label: t('admin.actions'), align: 'center' as const,
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}
        title={editingOption ? t('admin.edit_option') : t('admin.add_new_option')}
        subtitle={editingOption ? t('admin.update_option_info') : t('admin.add_option_subtitle')}
        size="md"
        footer={<>
          <Button variant="secondary" onClick={() => setIsModalOpen(false)} disabled={saving}>{t('common.cancel')}</Button>
          <Button variant="primary" onClick={handleSave} loading={saving}>{editingOption ? t('admin.update') : t('admin.create')}</Button>
        </>}>
        <div className="space-y-4">
          <FormInput label={t('admin.option_name')} placeholder={t('admin.option_placeholder_name')} value={formData.name}
      ...
      <Modal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} title={t('admin.option_confirm_delete')} size="sm"
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            {deletingOption && <p className="text-sm font-semibold text-[#181818] dark:text-white mt-2">{deletingOption.name}</p>}
          </div>
        </div>
      </Modal>
    </PageLayout>
