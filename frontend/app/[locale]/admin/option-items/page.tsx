...
export default function OptionItemsPage() {
  ...
  const handleSave = async () => {
    if (!isAdmin) { showAlertMessage(t('no_permission'), 'warning'); return }
    if (!formData.name.trim() || !formData.optionGroupId) { showAlertMessage(t('fill_required'), 'error'); return }
  ...
  return (
    <>
        ...
        <div className="bg-white dark:bg-dark-surface border border-light-gray-surface dark:border-dark-surface rounded-sm p-6">
          <DataTable
            columns={[
              ...
              { key: 'name', label: t('option_item_name'), sortable: true },
              { key: 'optionGroupName', label: t('option_item_group'), sortable: true },
              { key: 'price', label: t('option_item_price'), render: (v: any) => v ? `$${v.toLocaleString()}` : '—' },
              ...(isAdmin ? [{
                key: 'actions' as keyof OptionItem, label: t('actions'), align: 'center' as const,
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}
        title={editingItem ? t('edit_option_item') : t('add_option_item')}
        size="md"
        footer={<>
          <Button variant="secondary" onClick={() => setIsModalOpen(false)} disabled={saving}>{tCommon('cancel')}</Button>
          <Button variant="primary" onClick={handleSave} loading={saving}>{editingItem ? t('update') : t('create')}</Button>
        </>}>
        <div className="space-y-4">
          <FormInput label={t('option_item_name')} value={formData.name}
            onChange={(e) => setFormData({ ...formData, name: e.target.value })} required />
          
          <div className="space-y-2">
            <label className="block text-sm font-medium text-near-black dark:text-light-gray-surface">
              {t('option_item_group')} <span className="text-brand-red">*</span>
            </label>
            <select
              ...
              required
            >
              <option value="" disabled>{t('option_item_group')}</option>
              {groups.map((g) => (
          ...
          <FormInput label={t('option_item_price')} type="number"
